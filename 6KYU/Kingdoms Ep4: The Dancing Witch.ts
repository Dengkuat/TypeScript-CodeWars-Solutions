...
//aight so we iterate through the array of strings
//still again for each string inside the array we interate through again since l want to track each movement
//then if we have any space we continue and we have alot of spaces
//so we reduce the sums and they shoudl be 1 if it is greter the one then the nigga moved to much so we gatchu
//meaing that witch nigga we caught You
//we expose it and return it else if there is nothing then we have no witch we return null absence of a number
...
export function figureOut(arr: string[]): string | null {
  for (let i = 1; i < arr.length; i++) {
        ...
        for (const letter of previous) {
            ...
            const prevIndex = previous.indexOf(letter);
            const currIndex = current.indexOf(letter);
            const movement = Math.abs(currIndex - prevIndex);
